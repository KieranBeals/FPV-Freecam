...
public final class DroneInputMapper {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            if (controller.gamepadMapped()) {
                ...
                return new PollResult(
                        ...
                        togglePressed,
                        exitPressed,
                        this.readAxis(gamepadState, config.axisThrottle, config.deadzone, config.invertThrottle),
                        this.readAxis(gamepadState, config.axisYaw, config.deadzone, config.invertYaw),
                        this.readAxis(gamepadState, config.axisPitch, config.deadzone, config.invertPitch),
                        this.readAxis(gamepadState, config.axisRoll, config.deadzone, config.invertRoll)
                );
            }
            ...
            return new PollResult(
                    ...
                    togglePressed,
                    exitPressed,
                    this.readAxis(axes, config.axisThrottle, config.deadzone, config.invertThrottle),
                    this.readAxis(axes, config.axisYaw, config.deadzone, config.invertYaw),
                    this.readAxis(axes, config.axisPitch, config.deadzone, config.invertPitch),
                    this.readAxis(axes, config.axisRoll, config.deadzone, config.invertRoll)
            );
        }
    }
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            if (controller.gamepadMapped()) {
                ...
                for (int axis = 0; axis <= GLFW.GLFW_GAMEPAD_AXIS_LAST; axis++) {
                    final float value = Math.abs(gamepadState.axes(axis));
                    if (value > strongest) {
                        strongest = value;
                        result = axis;
                    }
            ...
            for (int axis = 0; axis < axes.limit(); axis++) {
                final float value = Math.abs(axes.get(axis));
                if (value > strongest) {
                    strongest = value;
                    result = axis;
                }
    ...
    private float readAxis(final GLFWGamepadState gamepadState, final int axis, final float deadzone, final boolean invert) {
        if (axis < 0 || axis > GLFW.GLFW_GAMEPAD_AXIS_LAST) {
            return 0.0F;
        }

        float value = gamepadState.axes(axis);
    ...
    private float readAxis(final FloatBuffer axes, final int axis, final float deadzone, final boolean invert) {
